...
if __name__ == "__main__":
    ...
    if cfg["question_type"] == "when_die":
        ...
        for category in data:
                ...
                assert category_type == "real_person"
                for char_data in characters:
                    char = char_data["role"]
                    death_year = char_data["year"]
                    source = char_data["source"]
    ...
    os.makedirs(f"answers/{cfg['type']}/claude", exist_ok=True)
    with open(f"answers/{cfg['type']}/claude/formatted_output_{cfg['type']}_{cfg['question_type']}.json", "w") as f:

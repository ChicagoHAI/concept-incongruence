...
def load_character_data():
    ...
    character_path = 'YOUR DATA PATH'
    
    with open(character_path, 'r') as f:
        return json.load(f)

def make_headline_prompt_dataset(short_prompt, prompt, tokenizer, entity_df, use_character=True):
    ...
    # breakpoint()
    # Add character names when flag is set and using when_w_period prompt
    if use_character and short_prompt == 'when_w_period':
        try:
            ...
            print(f"Warning: Could not add characters to headlines: {e}")

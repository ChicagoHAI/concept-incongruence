...
def load_tokenized_dataset(entity_type, prompt_name, model_family, prompt_type):
    save_path = prompt_data_path(entity_type, prompt_name, model_family, prompt_type)
    save_path = os.path.join('data', 'prompt_datasets', model_family, entity_type + prompt_type, prompt_name)
    return datasets.load_from_disk(save_path)


def prompt_data_path(entity_type, prompt_name, model_family, prompt_type):
    # breakpoint()
    return os.path.join(
        'data', 'prompt_datasets', model_family, entity_type + prompt_type, prompt_name
    )
...
class EntityDataManager:
    ...
    def prompt_data_path(self, prompt_name, model_family, prompt_type):
        return os.path.join(
            'data', 'prompt_datasets', model_family, self.entity_type + prompt_type, prompt_name
        )

    def load_tokenized_dataset(self, prompt_name, model_family, prompt_type):

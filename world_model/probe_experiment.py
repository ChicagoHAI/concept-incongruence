...
def save_probe_results(args, probe_results, prompt, pca=False):
    save_path = os.path.join(
        os.getenv('RESULTS_DIR', f'world_model/{args.output_type + "_results"}'),
...
def load_probe_results(
    ...
    if output_type is None:
        output_type = args.output_type + '_results'
    save_path = os.path.join(
        os.getenv('RESULTS_DIR', f'//world_model/{output_type}'),
        experiment_name,
        model,
        entity_type,
        feature_name,
...
def get_target_values(entity_df, feature_name):
    ...
    elif feature_name.endswith('date') or feature_name.endswith('year') or feature_name.startswith('president'):
        if feature_name == 'death_year':
            target = entity_df[feature_name].values
        elif feature_name.startswith('president'):
            target = entity_df['year'].values
        else:
...
RIDGE_ALPHAS = {
    ...
    'meta-llama/Llama-3.1-8B-Instruct': np.logspace(0.8, 4.1, 12),
    'google/gemma-2-9b-it': np.logspace(0.8, 4.1, 12),
}
...
def main_probe_experiment(args):
    ...
    for layer in tqdm.tqdm(range(n_layers)):
        # Print the activation file path
        dataset_dir = args.output_type + '_activation_datasets'
        print(f"Loading activations from: {dataset_dir} for model={args.model}, entity_type={args.entity_type}, prompt={args.prompt_name}, layer={layer}")
        
        # load data
        activations = load_activation_probing_dataset_args(
            args, args.prompt_name, layer, dataset_dir).dequantize()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--output_type', type=str, default='new_normal_results')
    # parser.add_argument(

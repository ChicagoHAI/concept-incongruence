...
def load_activation_probing_dataset_args(args, prompt_name, layer_ix, output_type=None):
    if output_type is None:
        output_type = args.output_type
    print(f"Loading activations from: world_model/{output_type} for model={args.model}, entity_type={args.entity_type}, prompt={prompt_name}, layer={layer_ix}")
    activation_save_path = os.path.join(
        os.getenv('ACTIVATION_DATASET_DIR', f'world_model/{output_type}'),
        args.model,
        args.entity_type
    )
    # breakpoint()
...
def load_activation_probing_dataset(model, entity_type, prompt_name, layer_ix, output_type=None, activation_aggregation='last'):
    if output_type is None:
        output_type = args.output_type
    activation_save_path = os.path.join(
        f'/world_model/{output_type}', model, entity_type)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--prompt_type', default='_new_normal')
    parser.add_argument('--output_type', type=str, default='new_normal_activation_datasets')
    ...
    for prompt_name in prompt_list:
        tokenized_dataset = common.load_tokenized_dataset(
            args.entity_type, prompt_name, model_family, args.prompt_type)
        ...
        n_seq, ctx_len = tokenized_dataset['input_ids'].shape
        print(f"Processing {n_seq} examples with context length {ctx_len}")

        activation_save_path = os.path.join(
            os.getenv('ACTIVATION_DATASET_DIR', f'/net/scratch2/smallyan/world_model/{args.output_type}'),
            args.model,
            args.entity_type
        )
